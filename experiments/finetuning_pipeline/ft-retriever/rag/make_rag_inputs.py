...
def clean(retriever, reranker):
    if reranker is None:
        if retriever == 'colbert':
            with open(f'../../../finetuning/colbert/50_positive/finetuned_colbert_50_positive_output_1000.json', 'r') as f:
                json_data = json.load(f)
        elif retriever == 'dpr':
            with open(f'../../../finetuning/dpr/200_positive/finetuned_dpr_200_positive_output_1000.json', 'r') as f:
                json_data = json.load(f)
        for fd_q_id, j_q in zip(test_set.keys(), json_data):
            ctxs = []
            for k in [1, 3, 5]:
                union_norm_context, union_freq_context = generate_top_k(j_q['ctxs'], k)
                ctxs.append({'id': f'{fd_q_id}_un_{k}', 'text': union_norm_context})
                ctxs.append({'id': f'{fd_q_id}_uf_{k}', 'text': union_freq_context})
            j_q['ctxs'] = ctxs
            j_q['id'] = fd_q_id
        save_path = f'./cleaned/retrievers'
        os.makedirs(save_path, exist_ok=True)
        save_path = f'{save_path}/{retriever}_output_1000.json'
        with open(save_path, 'w') as f:
            json.dump(json_data, f, indent=4)
    else:
        with open(f'../reranker/{retriever}/{reranker}.json', 'r') as f:
            json_data = json.load(f)
        for fd_q_id, j_q in zip(test_set.keys(), json_data):
            ctxs = []
            for k in [1, 3, 5]:
...
if __name__ == '__main__':
    ...
    for retriever in tqdm(retrievers):
        clean(retriever, None)

    for retriever in tqdm(retrievers):
        for reranker in rerankers:
            clean(retriever, reranker)
