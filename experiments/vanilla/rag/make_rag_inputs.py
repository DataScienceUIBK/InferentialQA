...
def clean(retriever, reranker):
    if reranker is None:
        with open(f'../retrievers/{retriever}/{retriever}_output_1000.json', 'r') as f:
        ...
        with open(f'../rerankers/{retriever}/{reranker}.json', 'r') as f:
            json_data = json.load(f)
        for fd_q_id, j_q in zip(test_set.keys(), json_data):
            ctxs = []
            for k in [1, 3, 5]:
...
if __name__ == '__main__':
    ...
    retrievers = ['bge', 'bm25', 'colbert', 'contriever', 'dpr']
