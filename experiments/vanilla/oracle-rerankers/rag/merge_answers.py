...
def merge(reranker):
    ...
    for model in ['meta-llama/Llama-3.2-1B-Instruct', 'google/gemma-3-4b-it', 'Qwen/Qwen3-8B']:
        ...
        with open(f'./qa_results/{reranker}/answers_{model_name}.json', 'r') as f:
    ...
    os.makedirs(f'./rag_results/', exist_ok=True)
    with open(f'./rag_results/{reranker}.json', 'w', encoding='utf-8') as f:
