...
for reranker_method in "${reranker_order[@]}"; do
  for reranker_model in ${rerankers[$reranker_method]}; do
    echo "Running: reranker_method=$reranker_method, reranker_model=$reranker_model"
    python reranker.py --reranker_method "$reranker_method" --reranker_model "$reranker_model"
  done
done
