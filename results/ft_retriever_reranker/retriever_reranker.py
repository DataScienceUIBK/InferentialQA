...
def hit_k(index_list, retriever_list, k, label_threshold=0, ignore_no_rels=True):
    ...
    for i in range(len(retriever_list)):
        ...
        retrieved = [c['id'] for c in retriever_list[i]['reranked_ctxs']][:k]
...
def evaluate(retriever, reranker):
    retriever_list = open_json(f'ft_retriever_reranker/{retriever}/{reranker}/reranker.json')
